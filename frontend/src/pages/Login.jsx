...
import { Shield, Key, ArrowRight, HelpCircle, ExternalLink, Loader2, AlertCircle } from 'lucide-react';
...
const Login = ({ onLogin }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (cleanPat.length === 0) return;
    
    setIsLoading(true);
    setError(null);

    try {
      const response = await fetch('https://giddily-status-energize.ngrok-free.dev/github-webhook', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${cleanPat}` 
        },
        body: JSON.stringify({ token: cleanPat }) 
      });

      if (!response.ok) {
        throw new Error('Invalid token or backend connection failed.');
      }
    ...
    <div className="min-h-screen bg-[#0F172A] flex flex-col justify-center items-center p-4">
      <div className="w-full max-w-md bg-[#1E293B] border border-slate-700/50 rounded-2xl shadow-2xl p-8 animate-in fade-in zoom-in duration-500">
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <button 
            type="submit"
            disabled={isLoading || pat.trim().length === 0}
